    ...
    if (!mounted) {
      return (
        <div className="rounded-[28px] border border-fg/10 bg-fg/[0.03] p-8 md:p-10">
          <div className="h-5 w-44 animate-pulse rounded-full bg-fg/10" />
          <div className="mt-5 h-7 w-72 animate-pulse rounded bg-fg/[0.07]" />
          <div className="mt-4 h-4 w-full max-w-md animate-pulse rounded bg-fg/5" />
        </div>
      );
    }
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {CONNECT_STEPS.map((s) => (
            ...
            >
              ...
              <p className="mt-2 text-sm leading-6 text-fg/70">{s.body}</p>
            </div>
          ))}
        </div>

        <div className="mt-8 rounded-[24px] border border-fg/12 bg-surface-2/60 p-6 md:p-8">
          <DlaBiurForm
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-[28px] border border-fg/10 bg-fg/[0.03] p-8 md:p-10">
        ...
        {integration.lastErrorMessage ? (
          ...
          <div className="mt-6 rounded-2xl border border-brand/20 bg-brand/10 p-4 text-sm leading-6 text-brand-text">
          ...
          <div className="mt-6 rounded-2xl border border-yellow-500/25 bg-yellow-500/10 p-4">
            ...
            {paymentsEnabled ? (
              <div className="mt-4">
                <a
                  href="/panel/pakiety"
                  className="inline-flex rounded-full bg-brand px-5 py-3 text-sm font-semibold text-black hover:opacity-90"
                >
                  Kup ogłoszenia
                </a>
              </div>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <button
            ...
            className="inline-flex rounded-full bg-brand px-5 py-3 text-sm font-semibold text-black transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
          >
                ...
                <tbody>
                  {logs.map((log) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-fg">{CRM_ACTION_LABELS[log.action] ?? log.action}</td>

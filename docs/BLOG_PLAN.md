# Plan artykułów bloga (backlog 100)

Zasada: w czacie do artykułów bierzemy następny niezaznaczony temat z listy, sprawdzamy w bazie,
że nie ma duplikatu, piszemy wg `docs/BLOG_ARTYKULY.md`, publikujemy, zaznaczamy `[x]`.

Już opublikowane (NIE pisać ponownie): MPZP (co to jest, jak sprawdzić przed zakupem), warunki
zabudowy WZ, księga wieczysta, działka rolna a budowlana, checklista zakupu, klasa gruntu (x2),
droga dojazdowa, uzbrojenie (sprawdzanie + koszt), podział działki (koszt), inwestowanie w ziemię.
...
- [x] Nowość w „Sprawdź działkę": wyszukiwanie po numerze ewidencyjnym (slug: sprawdz-dzialke-wyszukiwanie-po-numerze) [kategoria: Aktualności]

...
- [~] **P14. Lekki kreator dodawania (killer dla P27b). CZĘŚCIOWO ZROBIONE.** Zrobione: auto-zdjęcie ortofoto GUGiK z obrysem działki na canvasie, zł/m² na żywo, krok „Szczegóły" opcjonalny, drag&drop plików, pierwsza warstwa rozbicia monolitu (czyste UI do `src/components/dzialka-form/ui.tsx`). **2026-09-15: lokalizacja pierwsza + dane z dokładnej pinezki** (pomysł Pauli, w prostej formie po feedbacku Daniela): formularz jak wcześniej, krok „Lokalizacja" pierwszy, jedno krótkie zdanie pod mapą; kliknięcie działki na mapie w trybie dokładnym uzupełnia powierzchnię, przeznaczenie z planu, MPZP, tytuł i zdjęcie z lotu ptaka (pól wpisanych ręcznie nie rusza); numer działki dopinany przy zapisie tylko przy dokładnej pinezce stojącej na działce; w „Podstawach" krótka ramka cen z ofert i aktów RCN; bez zmian w bazie. Pierwsza wersja z wyszukiwarką numeru i kartą działki (6f782ac) cofnięta tego samego dnia: za dużo tekstu. Pliki: `src/lib/kreatorDzialki.ts` (+testy), `src/components/dzialka-form/PodpowiedzCeny.tsx`, `LocationPicker.tsx` (`onDokladnyPunkt`). **2026-09-18:** strona `/wycena-dzialki` („Sprawdź wartość swojej działki", krótki tekst) kończy się przyciskiem do `/sprzedaj?d=<identyfikator>`, a ten wczytuje działkę z ewidencji i uzupełnia dane jak po kliknięciu na mapie; pod raportem „Sprawdź działkę" link „To Twoja działka? Wystaw ją za darmo"; przy okazji naprawione zdjęcie z lotu ptaka (GUGiK wyłączył stary adres ortofotomapy). **Zostało:** baner z QR, wydzielenie kroków do osobnych komponentów stanowych, twarda mierzalność lejka (start→publikacja). [[project-strategia-nisza]] [[project-prywatni-pozyskanie]]
...
- [x] **P38 C1.** „Sprawdź działkę" na stronie głównej wysoko (spod bloga zaraz za „Wyróżnione oferty"). Po kilku iteracjach copy (2026-09-03) sekcja stoi tak: etykieta „Raport o działce", nagłówek mówiący co dostajesz („Co wolno tu zbudować i ile ta ziemia kosztuje"), lista sześciu haseł, przycisk, a obok DOWÓD: karta z prawdziwej działki w Rzgowie (plan 4MN, 278 zł/m² z ogłoszeń kontra 206 zł/m² z aktów) z linkiem do jej pełnego raportu. Zasada, która to rozstrzygnęła: sekcja ma pokazywać produkt, nie opowiadać o nim. Ptaszek marki wydzielony do `src/components/CheckIcon.tsx` i współdzielony z `/sprawdz-dzialke`. `app/page.tsx`. 2026-09-02.

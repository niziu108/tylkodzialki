...
- **AKTUALNY PUNKT (do wyboru, wg priorytetu Gwiazdy Północnej):**
  0. **P38 — narzędzie jako lejek (w toku od 2026-09-02).** Zrobione B1 (copy sekcji „Co dostajesz" jako sześć pytań kupującego) i B2 (przykładowy raport pod narzędziem). Następne: **C1 — sekcja „Sprawdź działkę" na stronie głównej**, potem C2 (agregaty gminne z cenami transakcyjnymi RCN) i C3 (dystrybucja).
...
- [~] **P38. „Sprawdź działkę" jako lejek, nie jako zakładka (tor B+C). W TOKU od 2026-09-02.** Narzędzie jest gotowe i mocne (ULDK + MPZP + plan ogólny gminy + wycena z ofert + trend), doszło do tego zbieranie **cen transakcyjnych z RCN** (`src/lib/rcn.ts`, `npm run rcn:backfill`, [[project_rcn_ceny_transakcyjne]]). Problem nie leży w danych, tylko w tym, że narzędzie się nie sprzedaje (strona wyliczała funkcje) i że z głównej nikt go nie widzi. Kroki:
  ...
  - [ ] **C1. Sekcja „Sprawdź działkę" na stronie głównej**, wysoko (nie pod blogiem).
  - [ ] **C2. SEO narzędzia: agregaty gminne z cenami transakcyjnymi (RCN).** Raport per działka zostaje `noindex` (26 mln działek = thin content); indeksujemy strony gmin z realnymi cenami z rejestru.

...
- [~] **P14. Lekki kreator dodawania (killer dla P27b). CZĘŚCIOWO ZROBIONE.** Zrobione: auto-zdjęcie ortofoto GUGiK z obrysem działki na canvasie, zł/m² na żywo, krok „Szczegóły" opcjonalny, drag&drop plików, pierwsza warstwa rozbicia monolitu (czyste UI do `src/components/dzialka-form/ui.tsx`). **2026-09-15: kreator „najpierw działka"** (pomysł Pauli): krok 1 = numer działki albo mapa z granicami, a lokalizacja, powierzchnia, przeznaczenie z planu, tytuł i zdjęcie z lotu ptaka uzupełniają się same; w kroku „Cena" podpowiedź z ofert i aktów RCN (kwota i procent tylko przy działkach podobnej wielkości); telefon w kroku „Kontakt"; sama miejscowość zostaje jako wyjście awaryjne; bez zmian w bazie. Pliki: `src/lib/kreatorDzialki.ts` (+testy), `src/components/dzialka-form/{ParcelFinder,KartaDzialki,PodpowiedzCeny}.tsx`. **Zostało:** strona „Sprawdź wartość swojej działki" wpięta w kreator, „To Twoja działka?" pod raportem, baner z QR, wydzielenie kroków do osobnych komponentów stanowych, twarda mierzalność lejka (start→publikacja). [[project-strategia-nisza]] [[project-prywatni-pozyskanie]]
